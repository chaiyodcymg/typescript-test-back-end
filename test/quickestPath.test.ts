...
describe("quickestPath function", () => {
    ...
    const laddersOtherBoards: [number, number][] = [ [12, 39], [14, 35], [31, 70], [46, 65], [49, 86], [63, 83], [71, 98] ];
    const snakesOtherBoards:  [number, number][] = [ [21, 4], [30, 8],  [45, 38], [79, 42], [87, 54], [91, 48], [96, 66] ];
    ...
    test(`quickestPath should be matched cases ladders and snakes are other boards`, () => {
        const result: number[] = quickestPath({ladders: laddersOtherBoards , snakes: snakesOtherBoards})
        const findResult = casesOtherBoards.some((value) => JSON.stringify(value) == JSON.stringify(result))
        expect(findResult).toEqual(true)
    })
});